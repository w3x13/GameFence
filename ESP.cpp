...
void CEsp::DrawBombus(IClientEntity* pEntity) {
	...
	{
		Render::nonamegey(vScreen.x, vScreen.y, "BOMB", Render::Fonts::esp, Color(255, 255, 255));
	}
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		if (lifetime > 0.01f && !Bomb->IsBombDefused())
		{
			...
			Render::gradient_horizontal(0, 2, 20, boomval, Color(Options::Menu.ColorsTab.bomb_timer.GetValue()), Color(10, 10, 10, 55));
...
void CEsp::DrawAmmo(IClientEntity* pEntity, Box size)
{
	...
	Render::rect(size.x, size.y + size.h + 2, size.w, 2, Color(2, 201, 255, 255));
	Render::rect(size.x, size.y + size.h + 2, ammoyes, 2, arc);
	Render::Text((size.x + 1) + ammoyes, (size.y - 3.5)+ size.h, Color(2, 201, 255, 255), Render::Fonts::smallassfont, std::to_string(pWeapon->GetAmmoInClip()).c_str());
...
void CEsp::DrawWeaponIcon(IClientEntity* pEntity, Box size)
{
	...
	if (pWeapon)
	{
		if (Options::Menu.VisualsTab.Icons.GetState()) {
...
void CEsp::DrawPlayer(IClientEntity* pEntity, player_info_t pinfo)
{
	...
	if (get_box(pEntity, box, Options::Menu.VisualsTab.Active.GetState()) && (pEntity->team() != hackManager.pLocal()->team()))
	{
		...
		if (Options::Menu.VisualsTab.OptionsHealth.GetIndex() != 0)
		{
			...
			{
			case 1: hp_default(pEntity, box), hp_text(pEntity, box);
				break;
			case 2: hp_battery(pEntity, box), hp_text(pEntity, box);
				break;
...
void CEsp::hp_text(IClientEntity* pEntity, Box size)
{
	...
	Render::Text(size.x + size.w + 5, (size.y + (text_side ? 10 : 0)) + (1 * Size.bottom) - dist, Color(30, 206, 6, 255), Render::Fonts::esp, HPBuffer);
...
void CEsp::ar_text(IClientEntity* pEntity, Box size)
{
	...
	Render::Text(size.x + size.w + 5, (size.y + (text_side ? 20 : 7)) + (1 * Size.bottom) - dist, Color(30, 206, 6, 255), Render::Fonts::esp, armorbuffer);
