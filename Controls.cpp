...
void CColorSelector::Draw(bool hover)
{
	...
	Render::rect(is_open && set_new_pos ? a.x + 184 : a.x, a.y, 16, 10, preview);
	Render::outlineyeti(is_open && set_new_pos ? a.x + 184 : a.x, a.y, 16, 10, Color(2, 2, 2, 255));
	...
	if (is_open && set_new_pos)
	{
		...
		_x += 5; _y += 5;
		Render::color_spectrum(_x, _y, 180, 180);
	}
}
...
void CColorSelector::OnUpdate() {
	...
	if (is_open && set_new_pos && GetAsyncKeyState(VK_LBUTTON))
	{
		int _x = a.x + 11;
		int _y = a.y + 22;
		RECT color_region = { _x, _y, 180, 180 };
		if (GUI.IsMouseInRegion(color_region))
		{
			color[0] = Render::color_spectrum_pen(_x, _y, 180, 180, Vector(GUI.GetMouse().x - _x, GUI.GetMouse().y - _y, 0)).r();
			color[1] = Render::color_spectrum_pen(_x, _y, 180, 180, Vector(GUI.GetMouse().x - _x, GUI.GetMouse().y - _y, 0)).g();
			color[2] = Render::color_spectrum_pen(_x, _y, 180, 180, Vector(GUI.GetMouse().x - _x, GUI.GetMouse().y - _y, 0)).b();
			color[3] = Render::color_spectrum_pen(_x, _y, 180, 180, Vector(GUI.GetMouse().x - _x, GUI.GetMouse().y - _y, 0)).a();
		}
	}
