...
void Hooks::Initialise()
{
	...
	Interfaces::Engine->ExecuteClientCmd("fps max 0");
	Interfaces::Engine->ExecuteClientCmd("fps_max 0");
	Interfaces::Engine->ExecuteClientCmd("clear");
	Interfaces::CVar->ConsoleColorPrintf(Color(50, 255, 50, 255), ("[GameFence] "));
	Interfaces::CVar->ConsoleColorPrintf(Color(255, 255, 255, 255), (" Injected Succesfully.\n\n---UPDATE LOG 7.2.2019---\n-Fixed CFG system\n-Redesigned menu\n-Added features\n-Fixed Color issues\n-Updated for the latest game update (7.2.2019)\n-Added No crouch cooldown\n"));
...
int __stdcall Hooked_DoPostScreenEffects(int a1)
{
	...
	for (auto i = 0; i < Interfaces::m_pGlowObjManager->size; i++)
	{
		...
		if (m_entity->GetClientClass()->m_ClassID == 38)
		{
			...
			if (m_entity->team() != m_local->team() && Options::Menu.VisualsTab.GlowZ.GetState() > 0)
			{
				...
				glow_object->m_vGlowColor = Vector(m_flRed / 255, m_flGreen / 255, m_flBlue / 255);
				glow_object->m_flGlowAlpha = Options::Menu.VisualsTab.GlowZ.GetState() / 100;
				glow_object->m_bRenderWhenOccluded = true;
				glow_object->m_bRenderWhenUnoccluded = false;
			}
...
std::string Tag = " gamefence "; // "freakratskids"
std::string Tag2 = " [gamefence.pub] ";
...
*/
	if (Options::Menu.ColorsTab.ClanTag.GetState())
	{
		if (!m_local)
		{
			Tag2 += Tag.at(0);
			Tag2.erase(0, 1);
			set_clan_tag(Tag2.c_str(), " gamefence ");
		}
		static size_t lastTime = 0;
		...
		if (GetTickCount() > lastTime)
		{
			Tag += Tag.at(0);
			Tag.erase(0, 1);
			set_clan_tag(Tag.c_str(), " [gamefence.pub] ");
			lastTime = GetTickCount() + 650;
		}
	...
	else if (FocusOverlayPanel == vguiPanel)
	{
		...
		if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame() && pLocal->isValidPlayer())
		{
			...
			if (Options::Menu.VisualsTab.manualaa_type.GetIndex() > 1)
			{
				...
				if (bigboi::indicator == 1)
				{
					drawThiccLine(cW + 45, cH, cW + 35 - rounding, cH - 10, 1, Color(48, 232, 23));// RIGHT | UP
																								   //RIGHT TARROW
					drawThiccLine(cW + 45, cH, cW + 35 - rounding, cH + 10, 1, Color(48, 232, 23)); //RIGHT DOWN
				...
				if (bigboi::indicator == 2)
				{
					drawThiccLine(cW - 45, cH, cW - 35 + rounding, cH - 10, 0, Color(48, 232, 23)); //LEFT | UP 
																									//LEFT ARROW
					drawThiccLine(cW - 45, cH, cW - 35 + rounding, cH + 10, 0, Color(48, 232, 23)); //LEFT | DOWN 
				...
				if (bigboi::indicator == 3)
				{
					drawThiccLine(cW, cH + 45, cW - 10, cH + 35 - rounding, 2, Color(48, 232, 23));// DOWN | LEFT
																								   //DOWN ARROW
					drawThiccLine(cW, cH + 45, cW + 10, cH + 35 - rounding, 2, Color(48, 232, 23)); // DOWN | RIGHT
				...
				if (bigboi::indicator == 4)
				{
					drawThiccLine(cW, cH - 45, cW + 10, cH - 35 + rounding, 2, Color(48, 232, 23));// UP | LEFT
																								   //UP ARROW
					drawThiccLine(cW, cH - 45, cW - 10, cH - 35 + rounding, 2, Color(48, 232, 23)); // UP| RIGHT
		...
		if (!strcmp(Event->GetName(), "player_hurt"))
		{
			...
			if (entityid == Interfaces::Engine->GetLocalPlayer())
			{
				...
				if (nUserID || nDead)
				{
					...
					if (Options::Menu.VisualsTab.DebugLagComp.GetState())
						draw_hitboxes(hurt, 30, 206, 6, 255, 1, 0);
					...
					std::string yyyyes = "hp";
					std::string yyyyyes = " left";
		...
		if (!strcmp(Event->GetName(), "item_purchase"))
		{
			...
			if (nUserID || nDead)
			{
				if (!(localplayer->GetIndex() == Interfaces::Engine->GetPlayerForUserID(nDead))) {
					...
					if (Options::Menu.VisualsTab.buylogs.GetState())
					{
